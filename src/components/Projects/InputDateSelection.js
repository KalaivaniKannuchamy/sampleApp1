...
const InputDateSelection = ({ title,textStyle,width, value,onSelect, containerStyle, errorMsg, labelStyle, compulsary }) => {
    ...
    return (
        <View style={[
            ...
            <TouchableOpacity activeOpacity={0.5} onPress={()=>{onSelect()}} style={styles.inputStyle}>
                <Text 
                  style={[ 
                    textStyle,
                      ...
                      { width : width ? width*0.7 : '75%'}
                          ...
                          {value == '' ? `--/--/----` : `${value}`}</Text>
                <Icon name={'calendar-outline'} size={20} color={PLACEHOLDER}/>
            </TouchableOpacity>
            {
                errorMsg == "" ?
                    <Text> </Text>
